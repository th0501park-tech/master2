...
4. **MLB 메이저리그**: AL/NL 30개 구단 디비전 순위, KST 한국시간 자동 변환, 실시간 이닝, 승·패·세이브 투수, 선발 매치업, 타자/투수 리더보드, 네이버 wbaseball gameId 연동, 공식 3D Statcast Gameday 중계

---

## ✨ 2. 핵심 기능

- **스마트 캐시 갱신 & 실시간 자동 폴링 (`startLivePolling`)**:
  - 무거운 전체 크롤링은 TTL 캐시(30분)를 유지하면서도, LIVE 진행 중인 경기가 있으면 새로고침 시 25초 주기 실시간 fetch로 최신 스코어/이닝 즉시 동기화
  - 경기 시작(KST) 기준 안전 종료 판정(야구 5.5시간, 축구 3.5시간 초과 시 자동 종료)으로 LIVE 오류 방어
  - 프론트엔드 30초 주기 무중단 자동 백그라운드 폴링 지원
