...
4. **MLB 메이저리그**: AL/NL 30개 구단 디비전 순위, KST 한국시간 자동 변환, 실시간 이닝 한국어 포맷팅(`8회초 1아웃`), 현재 마운드 투수 vs 타석 타자 실시간 표출, 승·패·세이브 투수, 선발 매치업, 타자/투수 리더보드, 네이버 wbaseball gameId 연동, 공식 3D Statcast Gameday 중계

---

## ✨ 2. 핵심 기능

- **서버 타임존(UTC/KST) 무결성 & 스마트 캐시 갱신 (`get_now_kst`)**:
  - 클라우드 서버(Render UTC)와 로컬 개발 환경(KST) 간의 시차로 인한 캐시 프리징 현상을 `get_now_kst()` 표준화 및 `diff < 0` 무효화 가드로 원천 차단
  - 무거운 전체 크롤링은 TTL 캐시를 유지하면서도, LIVE 진행 중인 경기가 있으면 25초 주기 실시간 fetch로 최신 스코어/이닝 즉시 동기화
  - 경기 시작(KST) 기준 안전 종료 판정(야구 5.5시간, 축구 3.5시간 초과 시 자동 종료)으로 LIVE 오류 방어
  - 프론트엔드 스마트 백그라운드 자동 폴링: LIVE 경기 시 25초, 비경기 시간대 90초 주기 자동 갱신
- **MLB 실시간 LIVE 정밀 판정 & 실시간 투수/타자 매치업**:
  - MLB API `statusCode`(`O`, `F`, `CR`, `FR`) 및 `detailedState`(`Game Over`, `Final`) 검증으로 종료된 경기가 LIVE로 남지 않도록 정밀 종료 판정
  - 실시간 LIVE 이닝 포맷 한국어 정밀 매핑 (`8회초 1아웃`, `8회말` 등)
  - 공식 linescore에서 현재 마운드 투수(`current_pitcher`) 및 타석 타자(`current_batter`)를 실시간 추출하여 카드에 직관적으로 시각화
  - 종료 경기 25개, 예정 경기 16개로 노출을 대폭 확대하여 당일 전 경기 결과와 다음날 일정 완벽 제공
- **실시간(LIVE) 공식 중계센터 모달 (`live-relay-modal`)**:
  - 경기가 실시간(LIVE)으로 진행 중일 경우 하단 버튼이 **`[🔴 실시간 중계확인]`** 버튼으로 자동 전환
  - 클릭 시 외부 새 창 없이 **웹페이지 내부의 깔끔한 단독 팝업(모달)**으로 즉시 열려 볼카운트, 투구 추적, 이닝별 상세 결과를 페이지 이탈 없이 확인 가능
  - 상단 시그니처 바로가기 버튼 제공: **[네이버 새 창 보기]** 및 **[MLB 3D게임데이 (Statcast 3D 투구궤적)]**
  - ESC 키 및 배경 클릭 닫기 완벽 지원
...
- **콘솔 오류 0(Zero) 무결성**:
  - Tailwind 경고 필터링, ESPN 공개 CDN 썸네일 매핑, iframe 리스너 안전 바인딩으로 브라우저 F12 콘솔 무결성 확보
